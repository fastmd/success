...
class ContractReport < Prawn::Document
  ...
  def to_pdf(contract,car,client)
    ...
    text "5.1. Chiria zilnică constituie <b>#{contract.price} Euro (#{pretperday} lei)</b>, respectiv pentru <b>#{daysinperiod}</b> zile. Locatarul urmează să achite <b>#{contract.summ} Euro (#{contract.costlei} lei)</b>.",:inline_format => true,:align => :justify
    ...
    text "I. Părțile contractante:", :align => :center, :style => :bold
    text "<b>SRL  Success & DivesGroup</b>  în  persoana  directorului  Sârbu  Sergiu  în  calitate  de  locator,  pe  de  o  parte,"+
         "  și  cet. <b>#{contract.client2.sname} #{contract.client2.name} #{contract.client2.fname}</b> în calitate de locatar, pe de altă parte, au convenit să încheie prezentul contract de Locațiune, cu respectarea următoarelor clauze: ",:inline_format => true,:align => :justify
    text "II. Obiectul contractului", :align => :center, :style => :bold
    text "Locatorul se obligă să transmită Locatarului în folosință temporară, cu titlu oneros, următorul automobil:" 
    move_down 1
    y_position = cursor
    ...
    bounding_box([260, y_position], :width => 260) do
       text " Locatar", :align => :center, :style => :bold
       move_down 3
       text " N.P. <font size='10'>#{contract.client2.sname} #{contract.client2.name} #{contract.client2.fname}</font>", :align => :left, :style => :bold,:inline_format => true
       text " ADRESA.  <font size='10'>#{contract.client2.address} </font>", :align => :left, :style => :bold,:inline_format => true
       text " SERIA.  <font size='10'>#{contract.client2.pseria} </font>", :align => :left, :style => :bold,:inline_format => true
       text " COD.IDNO.  <font size='10'>#{contract.client2.idno} </font>", :align => :left, :style => :bold,:inline_format => true
       text " D.N.  <font size='10'>#{contract.client2.bdate}</font>", :align => :left, :style => :bold,:inline_format => true
       text " D.E.  <font size='10'>#{contract.client2.dn}</font>", :align => :left, :style => :bold,:inline_format => true
       text " EMAIL.  <font size='10'>#{contract.client2.pemail}</font>", :align => :left, :style => :bold,:inline_format => true
       text " TEL.  <font size='10'>#{contract.client2.tel}</font>", :align => :left, :style => :bold,:inline_format => true
    end  #bounding box
    y_position = cursor
    bounding_box([0, y_position], :width => 260, :height => 40) do
       move_down 10
