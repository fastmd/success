...
class ContractReport < Prawn::Document
  ...
  def to_pdf(contract,car,client)
    ...
    text "SERGIU    078777058", :size => 12, :style => :bold, :align => :right
    move_down(1)
    text "078777059", :size => 12, :style => :bold, :align => :right
    text "CONTRACT DE LOCAȚIUNE nr. #{contract.id}/#{contract.cnum} din #{contract.order_date.strftime("%d.%m.%Y")}", :size => 15, :style => :bold, :align => :center
    move_down(1)
